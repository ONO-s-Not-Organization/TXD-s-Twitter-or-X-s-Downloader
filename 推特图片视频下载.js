...
class Utils {
    ...
    static resolveLangCode(raw) {
        if (!raw) return 'en';
        let lang = String(raw).toLowerCase().replace('_', '-');
...
class StorageManager {
    ...
    async setSetting(key, value) {
        await StorageCompat.setVal(key, value);
        const field = StorageManager.SETTING_FIELDS[key];
        if (field) this[field] = value;
        else if (key === Config.QUERY_ID_STORAGE_KEY) this.queryId = value;
        else if (key === Config.BEARER_STORAGE_KEY) this.bearer = value;
    }
    ...
    async addHistory(infoObj) {
        ...
        if (this.history.length > Config.HISTORY_LIMIT) {
            this.history = this.history.slice(-Config.HISTORY_LIMIT);
            this._syncHistoryIds();
        }
...
class Downloader {
    constructor(env) {
        this.env = env;
        this.timeout_ms = Config.DOWNLOAD_TIMEOUT_MS;
    }
        ...
        return new Promise(resolve => {
            ...
            try {
                ...
                const handle = gmDl({
                    ...
                    ontimeout: r => finish(false, r || { error: 'timeout', code: 'TIMEOUT' })
                });
    ...
    async run(task) {
        ...
        for (let attempt = 0; attempt <= 2; attempt++) {
            ...
            try {
                ...
                if (result && result.ok) {
                    try { task.onload && task.onload(); } catch (e) { }
                    return;
                }
        ...
        try { task.onerror && task.onerror(lastError); } catch (e) { }
    }
}
...
class UIManager {
    ...
    getLang() {
        ...
        let code = Utils.resolveLangCode(document.documentElement.lang);
    ...
    tryClickBookmark(article) {
        ...
        let bookmarkBtn = article.querySelector('button[data-testid="bookmark"]');
    ...
    updateHistoryCount() {
        if (this.historyBtn) {
            let label = this.historyBtn.querySelector('label');
    ...
    showModal(startView = 'history') {
        ...
        let dialogLang = this.lang.dialog || {};

        let shell = this.createModalShell(dialogLang, () => currentView);
        let { wrapper, titleEl, backBtn, settingsBtn, clearBtn, historyContainer, settingsContainer } = shell;
    ...
    createModalShell(dialogLang, getView) {
        let wrapper = Utils.$el(document.body, 'div', 'tmd-modal-wrapper');
        wrapper.onclick = (e) => { if (e.target === wrapper) wrapper.remove(); };

        let dialog = Utils.$el(wrapper, 'div', 'tmd-modal-dialog');
        if (this.app.storage.theme === 'dark') dialog.classList.add('tmd-dark-theme');

        let header = Utils.$el(dialog, 'div', 'tmd-modal-header');
        let headerLeft = Utils.$el(header, 'div', 'tmd-modal-header-left');
        let backBtn = Utils.$el(headerLeft, 'button', 'tmd-icon-btn', null, Config.icon_svg.back);
        let titleEl = Utils.$el(headerLeft, 'h3', 'tmd-modal-title', '');
        let headerActions = Utils.$el(header, 'div', 'tmd-modal-actions');

        let langSelect = Utils.$el(headerActions, 'select', 'tmd-lang-select');
        [
            { val: 'auto', txt: 'Auto' },
            { val: 'en', txt: 'English' },
            { val: 'zh', txt: '简体中文' }
        ].forEach(l => {
            let opt = Utils.$el(langSelect, 'option', '', l.txt);
            opt.value = l.val;
        ...
        let themeBtn = Utils.$el(headerActions, 'button', 'tmd-icon-btn', null,
        ...
        let settingsBtn = Utils.$el(headerActions, 'button', 'tmd-icon-btn', null, Config.icon_svg.settings);
        settingsBtn.title = this.lang.settings || 'Settings';

        let clearBtn = Utils.$el(headerActions, 'button', 'tmd-icon-btn danger', null, Config.icon_svg.clear);
        clearBtn.title = dialogLang.clear_history || 'Clear History';

        let closeBtn = Utils.$el(headerActions, 'button', 'tmd-icon-btn', null, Config.icon_svg.close);
        closeBtn.onclick = () => wrapper.remove();

        let historyContainer = Utils.$el(dialog, 'div', 'tmd-modal-content');
        let settingsContainer = Utils.$el(dialog, 'div', 'tmd-modal-settings');
    ...
    renderHistoryView(historyContainer, titleEl, backBtn, settingsBtn, clearBtn, settingsContainer, updateView) {
        let histLen = this.app.storage.history.length;
    ...
    renderHistoryCards(historyContainer, updateView) {
        let tableLang = this.lang.table || {};
        let list = Utils.$el(historyContainer, 'div', 'tmd-card-list');
        [...this.app.storage.history].reverse().forEach(item => {
            let card = Utils.$el(list, 'div', 'tmd-card');
            if (item.thumb) {
                let img = Utils.$el(card, 'img', 'tmd-thumb');
                img.src = item.thumb;
            }
            let body = Utils.$el(card, 'div', 'tmd-card-body');
            Utils.$el(body, 'div', 'tmd-card-user', item.user || '-');
            Utils.$el(body, 'div', 'tmd-card-meta', `${item.type || '-'} · ${this.formatDt(item.time)}`);

            let actions = Utils.$el(card, 'div', 'tmd-card-actions');
            let goBtn = Utils.$el(actions, 'button', 'tmd-action-btn', tableLang.go || 'Go');
            let delBtn = Utils.$el(actions, 'button', 'tmd-action-btn del', tableLang.del || 'Delete');
    ...
    renderHistoryTable(historyContainer, updateView) {
        let tableLang = this.lang.table || {};
        let tableWrap = Utils.$el(historyContainer, 'div', 'tmd-table-wrapper');
        let table = Utils.$el(tableWrap, 'table', 'tmd-table');
        let thead = Utils.$el(table, 'thead');
        let trHead = Utils.$el(thead, 'tr');
        ['thumb', 'user', 'type', 'postTime', 'downTime', 'action'].forEach(k => {
            let th = Utils.$el(trHead, 'th');
            Utils.$el(th, 'div', 'tmd-th-inner', tableLang[k] || k);
        });

        let tbody = Utils.$el(table, 'tbody');
        [...this.app.storage.history].reverse().forEach(item => {
            let tr = Utils.$el(tbody, 'tr');
            let tdThumb = Utils.$el(tr, 'td');
            if (item.thumb) {
                let img = Utils.$el(tdThumb, 'img', 'tmd-thumb');
                img.src = item.thumb;
            } else {
                tdThumb.textContent = '-';
            }
            ...
            let tdAction = Utils.$el(tr, 'td');
            let goBtn = Utils.$el(tdAction, 'button', 'tmd-action-btn', tableLang.go || 'Go');
            let delBtn = Utils.$el(tdAction, 'button', 'tmd-action-btn del', tableLang.del || 'Delete');
            this.bindHistoryItemActions(goBtn, delBtn, item, updateView);
        });
    }
    ...
    bindFilenamePatternUI(settingsContainer, dialogLang, wrapper) {
        ...
        let top_settings_row = Utils.$el(settingsContainer, 'div', 'tmd-pattern-header');
        top_settings_row.style.alignItems = 'center';

        let left_checkbox_group = Utils.$el(top_settings_row, 'div');
        left_checkbox_group.style.display = 'flex';
        left_checkbox_group.style.flexDirection = 'column';
        left_checkbox_group.style.gap = '10px';

        let save_history_label = Utils.$el(left_checkbox_group, 'label', 'tmd-checkbox-label');
        save_history_label.style.marginBottom = '0';
        let save_history_input = Utils.$el(save_history_label, 'input');
        ...
        let auto_bookmark_label = Utils.$el(left_checkbox_group, 'label', 'tmd-checkbox-label');
        auto_bookmark_label.style.marginBottom = '0';
        let auto_bookmark_input = Utils.$el(auto_bookmark_label, 'input');
        ...
        if (!this.app.env.isMobile) {
            let shortcut_label = Utils.$el(top_settings_row, 'div', 'tmd-pattern-label');
        ...
        let pattern_header = Utils.$el(settingsContainer, 'div', 'tmd-pattern-header');
        pattern_header.style.marginTop = '20px';
        Utils.$el(pattern_header, 'label', 'tmd-pattern-label', dialogLang.pattern || 'File Pattern');
        let resetBtn = Utils.$el(pattern_header, 'button', 'tmd-btn-reset', dialogLang.reset || '(Reset)');

        let pattern_input = Utils.$el(settingsContainer, 'textarea', 'tmd-textarea');
        let tag_container = Utils.$el(settingsContainer, 'div', 'tmd-tag-container');

        const tagsDict = dialogLang.tags || {};
        const excludedTags = ['{media-count}', '{date-time}'];
        let validTagsKeys = Object.keys(tagsDict).filter(tag => !excludedTags.includes(tag));

        validTagsKeys.forEach(tagText => {
            let tagBtn = Utils.$el(tag_container, 'div', 'tmd-available-tag', tagsDict[tagText]);
            tagBtn.onclick = () => {
        ...
        let preview_title = Utils.$el(settingsContainer, 'div', 'tmd-pattern-label', dialogLang.preview || 'Preview:');
        preview_title.style.marginTop = '15px';
        let preview_box = Utils.$el(settingsContainer, 'div', 'tmd-preview-box');
        let preview_error = Utils.$el(settingsContainer, 'div', 'tmd-preview-error', dialogLang.empty_pattern || 'Empty');
        let saveSettingsBtn = Utils.$el(settingsContainer, 'button', 'tmd-btn-save', dialogLang.save || 'Save');
    ...
    addButtonsToArticle(article) {
        ...
        let status_id = statusLink ? this.extractStatusId(statusLink.href) : null;
        ...
        if (media) {
            ...
            let group = article.querySelector('div[role="group"]:last-of-type, ul.tweet-actions, ul.tweet-detail-actions');
            if (status_id && group) {
                let shareCandidates = Array.from(group.querySelectorAll(':scope>div>div, li.tweet-action-item>a, li.tweet-detail-action-item>a'));
                let lastShare = shareCandidates.pop();
                if (lastShare && lastShare.parentNode) {
                    let btn_share = lastShare.parentNode;
        ...
        if (imgs.length > 1) {
            ...
            imgs.forEach(img => {
                ...
                let btn_down = this.createMediaDownloadBtn('tmd-img', 'download', this.lang.download);
    ...
    addButtonsToMediaList(listitems) {
        listitems.forEach(li => {
            ...
            let status_id = this.extractStatusId(statusLink.href);
            if (!status_id) return;

            let is_exist = this.app.storage.isDownloaded(status_id);
            let btn_down = this.createMediaDownloadBtn(
                'tmd-media',
...
class TwitterMediaDownloaderApp {
    constructor() {
        ...
        this.downloader = new Downloader(this.env);
    ...
    async handleDownloadClick(btn, status_id, index, retweeter_name = 'unknown', retweeter_id = 'unknown') {
        ...
        let out = (this.storage.filenamePattern || Config.defaultFilename).split('\n').join('');
        ...
        medias.forEach((media, i) => {
            ...
            let mediaUrl = picked.primary;
            let file = mediaUrl.split('/').pop().split(/[:?]/)[0];
            let fileName = file.split('.')[0];
            let fileExt = file.split('.').pop() || (media.type === 'photo' ? 'jpg' : 'mp4');
            let fileType = (media.type || 'photo').replace('animated_', '');
            let mediaIndex = index ? index : (i + 1);
            ...
            let indexSuffix = ((medias.length > 1 || index) && !out.includes('{index}') && !out.includes('{file-name}'))
                ? ('-' + mediaIndex)
                : '';
            let outName = Utils.buildFilename(out, fileInfo, { indexSuffix });
